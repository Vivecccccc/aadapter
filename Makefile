...
build-all:
	...
	GOOS=darwin GOARCH=amd64 CGO_ENABLED=0 go build -trimpath -ldflags "-s -w" -o dist/$(APP_NAME)_$(VERSION)_darwin_amd64 .
	GOOS=darwin GOARCH=arm64 CGO_ENABLED=0 go build -trimpath -ldflags "-s -w" -o dist/$(APP_NAME)_$(VERSION)_darwin_arm64 .
